...
```{r unmapped counts, echo=FALSE, message=FALSE}
...
```{r ADT counts across samples, echo=FALSE, message=FALSE, fig.height=5, fig.width=5}
...
adt_counts <- dplyr::bind_cols(adt_counts)
ADTs <- unlist(strsplit(rownames(res), "-"))
ADTs <- ADTs[seq(2, length(ADTs), 2)]
adt_counts$ADT <- ADTs
adt_counts <- melt(adt_counts)
ggplot(adt_counts, aes(x=ADT, y=value)) +
  geom_boxplot() +
  geom_jitter(width=0.15) +
  theme_bw() +
  coord_flip() +
  ylab("ADT sequence") +
  xlab("count (sum across cells)")
```
